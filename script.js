...
function renderProducts() {
  ...
  Object.entries(products).forEach(([catKey, cat]) => {
    ...
    if (catKey === 'V60') {
      ...
      // ✅ إصلاح: grid.appendChild(card) داخل الحلقة، section.appendChild(grid) بعدها
      cat.items.forEach(item => {
        ...
        grid.appendChild(card); // ✅ داخل الحلقة
      });

      section.appendChild(grid); // ✅ بعد الحلقة
...
function sendEmailNotification(orderData) {
  // ملاحظة: يحتاج backend أو خدمة مثل Firebase / EmailJS
